...
stands. `README.md` is the public runbook. `AGENTS.md` holds the working agreement,
including the teach rule, the command labels, and the build-log entry shape.

Section headings here are deliberately unnumbered so that anchors such as `#status`
stay stable when sections are added or removed.
...
- TLS before the site is public, real credentials are used, or content is indexed.
  The interim IP-only phase under *Status* is explicitly not a public launch.
...
- [x] **Step 0 — initialise the Git repository and ignore rules.**
...
The architecture decisions above assume code is versioned locally and deployed
through Git, and both documents forbid committing secrets. The build log lives in
these files, so this comes before any AWS work.

Complete. The commands that ran and the evidence are in the `README.md` build log.

Verification criteria, reworded after the fact because the original was not durable:
`git ls-tree -r --name-only HEAD` lists `.gitignore`, `AGENTS.md`, `README.md`, and
the `theme/` prototype, no tracked path matches a credential pattern, and
`git check-ignore` confirms each ignore rule bites. The original criterion also
required a clean tree, which only holds in the instant after a commit and fails
whenever a phase is in flight, so it is not a usable test of whether Step 0 was
done.
...
The command labels used throughout this file, and the read-only evidence required
before a step is marked complete, are defined in `AGENTS.md` under
